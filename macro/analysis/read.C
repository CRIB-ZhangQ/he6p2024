...
void read() 
{
    ...
    tree_out->Branch("fA", &fA, "fA/D");
    tree_out->Branch("fB", &fB, "fB/D");
    ...
    for(Int_t i = 0; i < entries; i++){
        fA = 0.0, fB = 0.0;
        tar_X = 0.0, tar_Y = 0.0, tar_Z = 0.0;
     mwdca_x = 0.0, mwdca_y = 0.0, mwdca_z = 0.0;
     mwdcb_x = 0.0, mwdcb_y = 0.0, mwdcb_z = -171.5;
        ...
        //test fA and fB
        Double_t dir_test[3] = {0.0};
        Double_t deltaX = -1.0 * fA * mwdcb_z;
        Double_t deltaY = -1.0 * fB * mwdcb_z;
        Double_t deltaZ = -1.0 * mwdcb_z;

        Double_t dis_test = sqrt(deltaX*deltaX + deltaY*deltaY + deltaZ*deltaZ);
        dir_test[0] = deltaX / dis_test;
        dir_test[1] = deltaY / dis_test;
        dir_test[2] = deltaZ / dis_test;
      //std::cout<<std::endl;
      //std::cout<<"deltaX="<<deltaX<<", deltaY="<<deltaY<<",  deltaZ="<<deltaZ<<std::endl;
      //std::cout<<dir_beam[0]<<"  "<<dir_beam[1]<<"  "<<dir_beam[2]<<std::endl;
      //std::cout<<dir_test[0]<<"  "<<dir_test[1]<<"  "<<dir_test[2]<<std::endl;
        //std::cout<<"res = "<<(dir_beam[0]-dir_test[0])/dir_beam[0]<<"  "<<(dir_beam[1]-dir_test[1])/dir_beam[1]<<"  "<<(dir_beam[2]-dir_test[2])/dir_beam[2]<<std::endl;
