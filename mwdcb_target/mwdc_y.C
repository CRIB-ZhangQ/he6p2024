void mwdc_y(){
  ...
  TH2F *h2 = (TH2F*)dirfile->Get("f3mwdcY_dcbY");
  if(!h2) {
  ...
  for(double x_center = x_min + delta/2.0; x_center<x_max; x_center=x_center+delta){
    ...
    TH1D *h1 = h2->ProjectionY("target_y", x_bin_min, x_bin_max); 
    ...
    h1->Draw();
    f_gaus->Draw("same");
    c1->SaveAs(Form("mwdcb_target/target_Y/mwdc_%d.png",i));
  ...
  legend_mu->AddEntry(gr_mu, "Target_y", "ap");
  legend_mu->AddEntry(f_mu, "Poly. fitting", "l");
  legend_mu->Draw();
  c1->SaveAs("mwdcb_target/target_Y/mwdc_mu.png");

//  TF1 *f_sigma = new TF1("pol_fit", "pol7(0)");
//  f_sigma->SetParameters(5, 1.6,0.06,-0.015,-0.001,0.0001,5.49e-6,-2.9e-7,-8.22e-9);
...
//  if (stats_sigma) {
//    stats_sigma->SetX1NDC(0.33); // 设置统计框的左下角 X 坐标
//    stats_sigma->SetY1NDC(0.1); // 设置统计框的左下角 Y 坐标
//    stats_sigma->SetX2NDC(0.7); // 设置统计框的右上角 X 坐标
//    stats_sigma->SetY2NDC(0.4); // 设置统计框的右上角 Y 坐标
//  }
//
//  TLegend *legend_sigma = new TLegend(0.1, 0.8, 0.4, 0.9);  // 调整这里以改变位置
//  legend_sigma->AddEntry(gr_sigma, "Target_y_sigma", "ap");
//  legend_sigma->AddEntry(f_sigma, "Poly. fitting", "l");
//  legend_sigma->Draw();
//  c1->SaveAs("mwdcb_target/target_Y/mwdc_sigma.png");
}
