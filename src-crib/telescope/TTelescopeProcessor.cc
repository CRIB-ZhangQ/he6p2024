...
void TTelescopeProcessor::Process() {
    ...
    for (Int_t iData1 = 0; iData1 < nData1; ++iData1) {
        ...
        //std::cout<<"dXID = "<<Data1->GetDetID()<<", XTiming = "<<Data1->GetTiming()<<std::endl;
        if (dEX_tmp < Data1->GetCharge()) { // overflow signal should have very large value
        //if (dEX_tmp < Data1->GetCharge() && IsValid(Data1->GetTiming())){ // overflow signal should have very large value
            dEXID = iData1;
    ...
    if (nData1 != 0 && dEXID != -1) {
        ...
        //std::cout<<"dXID = "<<Data1->GetDetID()<<", XTiming = "<<timing<<std::endl;
        if (fIsDSSSD) {
    ...
    for (Int_t iData2 = 0; iData2 < nData2; ++iData2) {
        ...
        /// need to update: care about overflow event more clevar
        if (dEY_tmp < Data2->GetCharge()) { // overflow signal should have very large value
            dEYID = iData2;
...
//        // judge if the event is pedestal or not
//        if (fHasDetPrm) {
//            if (energy < fDetParameter->GetPedestal(index) {
//                energy = 0.0;
//            }
//        }
//
...
// end
    ...
    for (Int_t iE = 0; iE < NE; iE++) {
        ...
        if (itr < 0) {
            ...
            //if(IsValid(Data3->GetTiming())){
            //  outData->PushEnergyArray(energy);
            //  outData->PushTimingArray(timing);
            //  E += energy;
            //  Etotal += energy;
            //}else{
            //  outData->PushEnergyArray(0);
            //  outData->PushTimingArray(kInvalidD);
            //}
        }
    }
